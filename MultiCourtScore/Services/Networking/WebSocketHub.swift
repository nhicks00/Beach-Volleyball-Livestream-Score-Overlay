...
.trad-name {
  ...
  text-overflow: ellipsis;
  min-width: 0;
  max-width: clamp(9rem, 18vw, 17.5rem);
...
<div id="overlay-wrapper" style="display: flex; flex-direction: column; align-items: center; width: 100%; max-width: 900px; padding: 0 1rem;">
  ...
  <div id="scorebug" class="carbon-bar" style="width: 100%; height: 4rem; border-radius: 0.375rem; display: flex; align-items: center; box-shadow: 0 8px 30px rgb(0,0,0,0.8); position: relative; overflow: hidden; z-index: 20;">
    ...
    <div id="scoring-content">
      <!-- Left Team Section -->
      <div id="team1-section" style="flex: 1; display: flex; align-items: center; justify-content: space-between; padding: 0 1.15rem; height: 100%; position: relative; overflow: hidden; min-width: 0;">
      ...
      <!-- Set Indicator -->
      <div id="set-indicator" style="padding: 0 1.2rem; display: flex; flex-direction: column; align-items: center; justify-content: center; min-width: 64px; flex-shrink: 0;">
      ...
      <!-- Right Team Section -->
      <div id="team2-section" style="flex: 1; display: flex; align-items: center; justify-content: space-between; padding: 0 1.15rem; height: 100%; position: relative; overflow: hidden; min-width: 0;">
...
function abbreviateName(teamName, opposingTeamName) {
  if (!teamName) return "";
  ...
  if (!normalizedTeamName.includes("/")) {
    ...
    if (normalizedTeamName.length <= 26) return normalizedTeamName;
    return compactWords(normalizedTeamName);
  }
  const players = normalizedTeamName.split("/").map(p => p.trim()).filter(Boolean);
  const parsedPlayers = players.map(parsePlayerLabel);
  const parsedOpponents = cleanName(opposingTeamName || '').includes("/")
    ? cleanName(opposingTeamName).split("/").map(p => p.trim()).filter(Boolean).map(parsePlayerLabel)
    : [];
  const surnameCounts = {};
  const initialCounts = {};
  const opponentInitialCounts = {};

  parsedPlayers.forEach(player => {
    if (!player.lastName) return;
    const surnameKey = player.lastName.toLowerCase();
    const initialKey = surnameKey + '|' + player.firstName.charAt(0).toLowerCase();
    surnameCounts[surnameKey] = (surnameCounts[surnameKey] || 0) + 1;
    initialCounts[initialKey] = (initialCounts[initialKey] || 0) + 1;
  });

  parsedOpponents.forEach(player => {
    if (!player.lastName) return;
    const surnameKey = player.lastName.toLowerCase();
    const initialKey = surnameKey + '|' + player.firstName.charAt(0).toLowerCase();
    opponentInitialCounts[initialKey] = (opponentInitialCounts[initialKey] || 0) + 1;
  });

  const sharedTeamLastName =
    parsedPlayers.length > 1 &&
    parsedPlayers.every(player => !player.placeholder && player.lastName && player.lastName.toLowerCase() === parsedPlayers[0].lastName.toLowerCase())
      ? parsedPlayers[0].lastName
      : '';

  if (sharedTeamLastName) {
    return parsedPlayers.map(player => player.firstName || player.display).join(" / ") + ' ' + sharedTeamLastName;
  }

  return parsedPlayers.map(player => {
    if (player.placeholder) return player.placeholder;
    if (!player.lastName) return player.display;

    const surnameKey = player.lastName.toLowerCase();
    const initialKey = surnameKey + '|' + player.firstName.charAt(0).toLowerCase();
    const initialCollisionAcrossSides = (opponentInitialCounts[initialKey] || 0) > 0;
    if (((surnameCounts[surnameKey] || 0) > 1 && (initialCounts[initialKey] || 0) > 1) || initialCollisionAcrossSides) {
...
function buildTeamDisplayPair(team1, team2) {
  return {
    team1: abbreviateName(team1, team2),
    team2: abbreviateName(team2, team1),
  };
}
...
function abbreviatePlaceholderName(name) {
  ...
  return compactBracketLabel(normalized);
}
...
function parsePlayerLabel(playerLabel) {
  const normalizedPlayer = cleanName(playerLabel);
  ...
  if (lower.includes("winner") || lower.includes("loser") ||
    ...
    return {
      display: normalizedPlayer,
      placeholder: abbreviatePlaceholderName(normalizedPlayer),
      firstName: '',
      lastName: ''
    };
  }

  const parts = normalizedPlayer.split(/\s+/).filter(Boolean);
  if (parts.length < 2) {
    return {
      display: normalizedPlayer,
      placeholder: '',
      firstName: normalizedPlayer,
      lastName: ''
    };
  }
...
function compactWords(label) {
  const words = cleanName(label).split(/\s+/).filter(Boolean);
...
function compactBracketLabel(label) {
  ...
  return label
    ...
    .replace(/\bQuarterfinal\b/gi, 'QF')
    .replace(/\bSemifinals?\b/gi, 'SF')
    .replace(/\bSemifinal\b/gi, 'SF')
    .replace(/\bChampionship\b/gi, 'Final')
    .replace(/\bCourt\s+([A-Za-z]+|\d+)\b/gi, 'Ct $1');
}
...
function resetFittedText(el) {
  if (!el) return;
  if (!el.dataset.baseFontSize) el.dataset.baseFontSize = window.getComputedStyle(el).fontSize;
  if (!el.dataset.baseLetterSpacing) el.dataset.baseLetterSpacing = window.getComputedStyle(el).letterSpacing;
  el.style.fontSize = el.dataset.baseFontSize;
  el.style.letterSpacing = el.dataset.baseLetterSpacing;
}

function fitTextToBox(el, minPx) {
  if (!el) return;
  resetFittedText(el);
  ...
  if (!available && el.parentElement) available = el.parentElement.clientWidth;
  ...
  if (el.scrollWidth > available) {
    el.style.letterSpacing = '-0.055em';
    void el.offsetWidth;
    while (el.scrollWidth > available && size > Math.max(minPx - 1.5, 8)) {
      size -= 0.5;
      el.style.fontSize = size + 'px';
      void el.offsetWidth;
    }
...
function fitCenterTeamNames() {
  fitTextToBox(t1El, 10.5);
  fitTextToBox(t2El, 10.5);
}

function fitIntermissionTeamNames() {
  fitTextToBox(intTeam1, 14);
  fitTextToBox(intTeam2, 14);
}

function getMaxTradNameWidth() {
  return Math.max(150, Math.min(Math.round(window.innerWidth * 0.18), 280));
}

function fitTradTeamNames() {
  fitTextToBox(tradT1, 10);
  fitTextToBox(tradT2, 10);
}
...
function showNextMatchBar(nextMatchText) {
  ...
  if (nextTeamsEl && nextMatchText) {
    ...
    if (parts.length >= 2) {
      const display = buildTeamDisplayPair(parts[0], parts[1]);
      nextTeamsEl.innerHTML = display.team1 + ' <span style="color: rgba(255,255,255,0.4); font-style: italic; margin: 0 4px; font-weight: 900;">vs</span> ' + display.team2;
    } else {
...
function applyData(d) {
  ...
  // Team names
  const displayNames = buildTeamDisplayPair(d.team1 || '', d.team2 || '');
  if (t1El) t1El.textContent = displayNames.team1 || 'Team 1';
  if (t2El) t2El.textContent = displayNames.team2 || 'Team 2';
  fitCenterTeamNames();
...
function animateMatchChange(newTeam1, newTeam2, newData) {
  ...
  const displayNames = buildTeamDisplayPair(newTeam1, newTeam2);
  
  // Pre-set the intermission team names (hidden)
  if (intTeam1) {
    intTeam1.textContent = displayNames.team1 || 'TBD';
    intTeam1.style.transform = 'translateX(30px)';
    intTeam1.style.opacity = '0';
  }
  if (intTeam2) {
    intTeam2.textContent = displayNames.team2 || 'TBD';
...
function showIntermissionImmediate(team1, team2) {
  const displayNames = buildTeamDisplayPair(team1, team2);
...
function setIntermissionWidth() {
  ...
  const maxTeamWidth = Math.min(Math.max(team1Width, team2Width), Math.max(180, Math.round(window.innerWidth * 0.26)));
...
function transitionToIntermission(nextTeam1, nextTeam2) {
  ...
  clearCelebration();
  const displayNames = buildTeamDisplayPair(nextTeam1, nextTeam2);
...
function applyLayoutTransition(newLayout) {
  ...
  var shouldRetriggerTradBubbleEntrance = previousLayout !== 'center' && newLayout !== 'center';
  var visibleTradBubbles = shouldRetriggerTradBubbleEntrance ? {
    social: socialBar.classList.contains('visible'),
    next: nextBar.classList.contains('visible'),
    status: !!(intStatusBar && intStatusBar.classList.contains('visible'))
  } : null;
  ...
  if (visibleTradBubbles) {
    if (visibleTradBubbles.social) {
      socialBar.classList.remove('visible');
      socialBar.classList.add('hidden-up');
    }
    if (visibleTradBubbles.next) {
      nextBar.classList.remove('visible');
      nextBar.classList.add('hidden-up');
    }
    if (visibleTradBubbles.status && intStatusBar) {
      intStatusBar.classList.remove('visible');
      intStatusBar.classList.add('hidden-up');
    }
  }
  ...
  setTimeout(function() {
    ...
    requestAnimationFrame(function() {
      ...
      requestAnimationFrame(function() {
        ...
        var finishLayoutTransition = function() {
...
function applyTradData(d) {
  ...
  // Team names
  var displayNames = buildTeamDisplayPair(d.team1 || '', d.team2 || '');
  var name1 = displayNames.team1 || 'Team 1';
  var name2 = displayNames.team2 || 'Team 2';
...
function equalizeTradNameWidths(force) {
  ...
  var targetWidth = Math.min(maxW, getMaxTradNameWidth());
  tradT1.style.minWidth = targetWidth + 'px';
  tradT2.style.minWidth = targetWidth + 'px';
  fitTradTeamNames();
}
      ...
      if (overlayState === 'scoring' || overlayState === 'postmatch') {
        ...
        if (intContent) {
          ...
          setTimeout(function() {
            ...
            const displayNames = buildTeamDisplayPair(newTeam1, newTeam2);
            if (intTeam1) intTeam1.textContent = displayNames.team1 || 'TBD';
            if (intTeam2) intTeam2.textContent = displayNames.team2 || 'TBD';
            setIntermissionWidth();
