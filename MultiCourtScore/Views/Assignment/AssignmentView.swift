...
struct AssignmentView: View {
    ...
    private func autoAssignMatches() {
        // Priority courts that should go to Core 1 (overlay 1)
        let priorityCourtPatterns = ["stadium", "center", "main", "feature", "show"]
        
        // Separate priority courts from numbered courts
        ...
        for courtName in groupedByCourt.keys {
            ...
            // Check if it's a priority court (stadium, center, etc.)
        ...
        // Sort numbered courts by their number
        numberedCourts.sort { $0.number < $1.number }
        
        // Assign priority courts (Stadium, Center) to Core 1
        for courtName in priorityCourts {
            if let matches = groupedByCourt[courtName] {
                for match in matches {
                    matchAssignments[match.id] = 1  // Core 1
                }
            }
        }
        
        // Assign numbered courts: Court 1 → Mevo 1 (overlay 2), Court 2 → Mevo 2 (overlay 3), etc.
        for (index, court) in numberedCourts.enumerated() {
            let overlayId = min(index + 2, AppConfig.maxCourts)  // Start at overlay 2 (Mevo 1)
        ...
        // Assign remaining courts (letters like A, B, C) to next available overlays
    ...
    private func importAssignedMatches() {
        ...
        // Sort matches within each overlay: Pool matches first, then Bracket matches
        for (overlayId, matches) in matchesByOverlay {
            let sortedMatches = matches.sorted { a, b in
                ...
                // Pool matches come first
                if aIsPool && !bIsPool { return true }
                if !aIsPool && bIsPool { return false }
                
                // Within same type, maintain existing order (by time/match number)
                return false
            }
            
            let matchItems = appViewModel.scannerViewModel.createMatchItems(from: sortedMatches)
